 ...
 */
 ...
 class PreorderRepository extends EntityRepository {
	...
	public function getPreorders( $criteria ) { 
		...
		$qb->select('b')
			->from('models\Entities\PreorderUser', 'b')//TODO leftjoin products, locations, users
		...
		if( $data['record_count'] = $data['records']->count() ) {
			...
			foreach( $data['records'] as $preorder ) {
				...
				$preorderPage = TRUE;
				try {
				    $preorderPage = $preorder->getPreorder()->getTitle();
				...
				$preorderItem = TRUE;
				try {
				    $preorderItem = $preorder->getPreorderItem()->getProductName();
				} catch (\Doctrine\ORM\EntityNotFoundException $e) {
				    $preorderItem = NULL;
				}
				...
				$data['record_items'][] = array(
					...
					$preorder->getLocation()->getID(),
					$preorder->getUser()->getID(),
					get_class($preorder->getUser()) == 'models\Entities\User\Customer\Personal' ? 'Privatni' : 'Poslovni',
					get_class($preorder->getUser()) == 'models\Entities\User\Customer\Personal' ? $preorder->getUser()->getFirstName().' '.$preorder->getUser()->getLastName() : $preorder->getUser()->getCompanyName(),
					get_class($preorder->getUser()) == 'models\Entities\User\Customer\Personal' ? '' : $preorder->getUser()->getContactPerson(),
					$preorder->getUser()->getPhone(),
					$preorder->getUser()->getAddress(),
					$preorder->getUser()->getPostalCode()->getPostalCode(),
					$preorder->getUser()->getFormatedRegistrationDate(),
				);
			}
		}
