                ...
                <form id="promotionForm" method="post" action="<?= site_url( 'promotion_code/submit' ) ?>">
					...
					<input type="hidden" name="productid" id="productid" value="<?=@$id?>" />
                        <ul>
                            ...
                            <!--<li><label>Tip</label>
                                <div class="customComboHolder">
                                    ...
                                    <select name="type" id="type" >
                                        <option value="1" selected="selected" <?=$type_id == 1 ? 'selected' : ''?>>Ostalo</option> 
                                        <option value="2" <?=$type_id == 2 ? 'selected' : ''?>>Rođendanska čestitka</option>
                                    </select>
                                </div>
                            </li>-->
                            <li>
                            ...
                            <li>
                                ...
                                <input type="text"  name="discount" class="number-range" value="<?=@$discount?>" id="discount"  required />
                             ...
                             <li>
                            	...
                            	<div class="dropbox">
									...
									<p>
										<input name="brand[]" type="checkbox" value="<?=$brand->getID()?>" <?=(in_array($brand->getID(), $brands)) ? 'checked' : '' ?> /><label><?=$brand->getName()?></label>
									</p>
									<?php endforeach; ?>
                            	</div>
                            	...
                            	<div class="dropbox">
									...
									<p>
										<input name="category[]" type="checkbox" value="<?=$cat->getID()?>" <?=(in_array($cat->getID(), $categories)) ? 'checked' : '' ?> /><label><?=$cat->getName()?></label>
									</p>
									<?php endforeach; ?>
                            	</div>
                            	...
                            	<div class="dropbox">
									...
									<p>
										<input name="subcategory[]" type="checkbox" value="<?=$sub->getID()?>" <?=(in_array($sub->getID(), $subcategories)) ? 'checked' : '' ?> /><label><?=$sub->getName()?></label>
									</p>
									<?php endforeach; ?>
                            	</div>
                            </li>
                            ...
                            <li>
                                <label>Status:</label>
                                <input type="radio" name="status" value="1"  <?=@$status ? 'checked' : ''?> />
                                <small>Aktivna</small>
                                <input type="radio" name="status" value="0" <?=@$status ? '' : 'checked'?> />
                    ...
                    <fieldset class="sectionForm" style="width:226px; float:left;  padding-left: 20px;">
        				...
        				<?php foreach($products as $p): ?>
        					<?php if( $p != '' ): ?>
		        				<li>
		        					<label style="display:inline-block; width:40px;">ID:</label>
		        					<span class="inputField wide" style="display: inline-block; width:60px;">
		        						<input type="text" name="product[]" class="required input-product" style="width:40px;" maxlength="6" value="<?=$p?>">
		        					</span><a href="#" onclick="deleteProductItem(this);" style="margin-left:3px;top:5px; position:relative;"><img src="http://dev.codeion.com/kiddyjoy/site/assets/img/layout/_backend/flexigrid/delete.png"></a>
		        				</li>
		        			<?php endif; ?>
        				<?php endforeach; ?>
