...
class Flexigrid
{
	...
	*/	
	 ...
	 * @return	void
	 */
	public function validate_post($id, $d_sortname,$d_sortorder,$valid_fields = NULL)
	{
		...
		$grid_navigator[$id] = $this->post_info['page'];
		$this->CI->session->set_userdata('grid_navigator', $grid_navigator);
        ...
        // Codeion SA - Doctrine implementation
        $this->post_info['search_field'] = ($this->CI->session->userdata('qtype') != '') ? $this->CI->session->userdata('qtype') : $this->CI->input->post('qtype');
        $this->post_info['search_keyword'] = ($this->CI->session->userdata('query') != '') ? $this->CI->session->userdata('query') : $this->CI->input->post('query');
	}
    ...
    public function get_criteria() {
		...
		if (!isset($criteria)) $criteria = new stdClass();

        $criteria->search_field = $this->post_info['search_field'];
        $criteria->search_keyword = $this->post_info['search_keyword'];
        $criteria->sortname = $this->post_info['sortname'];
        $criteria->sortorder = $this->post_info['sortorder'];
        $criteria->limit = $this->post_info['rp'];
        $criteria->offset = $this->post_info['limitstart'];
