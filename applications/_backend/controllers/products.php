 ...
 */
 ...
 class Products extends MY_Controller {
    ...
    public function listing($category_id, $back_button = NULL) {
		...
		$buttons[] = array('Export proizvoda kojih nema na stanju', 'url', 'grid_commands', 'url:' .site_url("products/out_of_stock/" .  $category_id));
	...
	public function out_of_stock($group_id) {
		...
		$group_name = $group->getCategory()->getName();
		...
		/* EXCEL */
		$csv 	= 'Datum:' . $date . "\t" . strtoupper($group_name) . "\t" . strtoupper($subcategory) . "\t". '' . "\t" . ''. "\t" . ''. "\n";
		$csv 	.= '' . "\t" . '' . "\t" . '' . "\t" . '' . "\t" . '' . "\n";
		$csv 	.= 'ID' . "\t" . 'MASTER ID' . "\t" . 'BRAND' . "\t" . 'NAME' . "\t" . 'PRICE' . "\n";
		 ...
		 header('Content-Disposition: attachment; filename="'.$group_name.'_'.$subcategory.'_'.$date.'_outofstock.xls"');
